...
with mysql.connector.connect(host="localhost",user="root",password="system",database="flaskblog"):
    ...
    cursor.execute("create table if not exists posts(id int auto_increment ,title varchar(255),content text,date_posted datetime default current_timestamp,slug varchar(255) ,poster_id varchar(50),primary key(id),key poster_id(poster_id),constraint fk_poster_id foreign key (poster_id) references registrationforms (username))")
app=Flask(__name__)
...
def login():
    if request.method=="POST":
        ...
        password=request.form['password']
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select count(*) from registrationforms where username=%s && password=%s',[username,password])
